...
const fetchMainCategories = async () => {
  try {
    ...
    if (response.data.result === 'Success') {
      ...
      if (selectedCategoryIdFromParams) {
        ...
        if (foundCategory) {
          ...
          console.log('❌ Category not found in main categories, showing all');
          setSelectedCategoryId(null);
          // FIX: Always fetch all categories when category is not found or not specified
          await fetchCategories();
        }
      } else {
        // FIX: When no category ID is provided (coming from "More Materials"), fetch all categories
    ...
    console.error('Error fetching main categories:', error);
    Alert.alert('Error', 'Failed to load main categories');
    // FIX: Fallback to all categories on error
    await fetchCategories();
  }
};
