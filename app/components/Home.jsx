...
const Home = () => {
  ...
  // Tap debouncing function - MOVED TO TOP
  ...
  const handleSearchItemSelect = withTapPrevention((item) => {
  ...
  // Close search modal
  const closeSearchModal = withTapPrevention(() => {
    setSearchModalVisible(false);
    setSearchQuery('');
  });
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        setTimeout(() => {
          if (mounted) safeApiCall(fetchSearchData);
        }, 1500);
    ...
    if (userId) {
      ...
      try {
        router.push({ 
          ...
          params: { 
            selectedCategoryId: material.id.toString(),
